...
export default {
  ...
  methods: {
    ...
    processData() {
      ...
      let xnum = 20,
        ynum = 20;
      let xspan = (this.dataExtent[2] - this.dataExtent[0]) / xnum;
      let yspan = (this.dataExtent[3] - this.dataExtent[1]) / ynum;
      let xScale = d3
        .scaleQuantize()
        .domain([this.dataExtent[0], this.dataExtent[2]])
        .range(d3.range(0, xnum));
      let yScale = d3
        .scaleQuantize()
        .domain([this.dataExtent[1], this.dataExtent[3]])
        .range(d3.range(0, ynum));
      let grid = [];
      ...
      for (let i = 0; i < xnum; i++) {
        for (let j = 0; j < ynum; j++) {
          if(grid[i][j] < 10) {
            ...
            while(diff > 0) {
              let x = this.dataExtent[0] + _.random(i*xspan, (i+1)*xspan, true);
              let y = this.dataExtent[1] + _.random(j*yspan, (j+1)*yspan, true);
              this.alldata.points.push([x, y]);
              diff--;
            }
          }
    ...
    addDocPoint() {
      ...
      for (let i = 0, len = projdata.length; i < len; i++) {
        ...
        let feature = new ol.Feature({
          geometry: new olgeom.Point(this.alldata.points[i]) //this.alldata.points[i]
      ...
      for (let i = projdata.length, len = this.alldata.points.length; i < len; i++) {
        // let center = d3.polygonCentroid(this.alldata.polygons[i]);
        let feature = new ol.Feature({
          geometry: new olgeom.Point(this.alldata.points[i]) //this.alldata.points[i]
          // geometry: new olgeom.Point([parseFloat(pg.x), parseFloat(pg.y)])
