...
const longdisHighsimilarity = require("./dist2similarity2.js")
const Graph = require("./dijkstra.js")
const projdata = require("../../../public/data/output/finance/projection_dense_tfidf.json")
const similarityMatrix = require("../../../public/data/output/finance/similarity_matrix_5round.json")
const cluserdata = require("../../../public/data/output/finance/cluster.json")

/**
 * 获取投影数据的范围
 * @param {Array} projdata [[x, y], ...]
 * @returns {Object} {
...
function generateOuterPoints(dataExtent, mapExtent, pointNum) {
  ...
  for (let i = 0; i < pointNum; i++) {
    ...
    if (tmp == 0) {
      ...
      outerPoints.push([+x.toFixed(5), +y.toFixed(5)]);
    }
  }
  return outerPoints;
}
    ...
    for (let j = 0; j < innerYNum; j++) {
      if (grid[i][j] < 10) {
        ...
        while (diff > 0) {
          ...
          innerPoints.push([+x.toFixed(5), +y.toFixed(5)]);
          diff--;
        }
      }
    }
...
function getVoronoi(mapExtent, allPoints, mapIterationNum) {
  ...
  for (let i = 0; i < mapIterationNum; i++) {
    ...
    let centerPoints = [];
    polygons.forEach(d => {
      let center = d3.polygonCentroid(d);
  ...
  polygons = polygons.map(pg => {
    return pg.map(p => [+p[0].toFixed(5), +p[1].toFixed(5)])
  })
  return polygons;
}
...
/**
 * 转换聚类数据格式
 * @param {Object} clusterdata [label,label,...]
 * @returns {Object} {label: [index1, index2, ...], ...}
 */
function getCluster(clusterdata) {
  let cluster = {};
  clusterdata.forEach((label, index) => {
    if (label in cluster) {
      cluster[label].push(index);
    } else {
      cluster[label] = [index];
    }
  })
...
function processMapData(projdata, similarityMatrix, clusterdata, config) {
  ...
  let allPoints = projdata.concat(outerPoints).concat(innerPoints);
  let pointIndexInfo = {
  ...
  let finalPoints = polygons.map(d => d3.polygonCentroid(d))
  let clusters = getCluster(clusterdata);
  let {
    ecoords,
    ecoords2index,
    edge2docindex
  } = getAllEdges(polygons, pointIndexInfo);
...
let writePath = "./public/data/output/finance/mapdata.json"
